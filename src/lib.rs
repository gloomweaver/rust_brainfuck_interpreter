...
impl Interpreter {
    ...
    fn parse_to_commands(&mut self, commands_string: String) -> () {
    ...
    fn get_input(&mut self, input: String) -> () {
        self.input = input.chars().rev().collect::<String>();
    }
    ...
    pub fn run(&mut self, commands_string: String, input: String) {
    ...
    fn input(&mut self) {
        ...
        if input_char.is_some() {
            ...
            if value as u8 > 0 {
                self.memory[self.pointer] = input_char.unwrap() as u8;
            }
        }
    }
    ...
    fn output(&mut self) {
        ...
        self.output.push(output_char as char)
    }
